...
class MediaPipeAdapter:
    ...
    def create_landmarker(self, options: PoseInferenceOptions) -> Any:
        ...
        for delegate_name in delegates:
            try:
                with _patched_platform_for_mediapipe_windows_queries():
    ...
    def _ensure_mediapipe(self) -> None:
        if self._mp is not None:
            return
        try:
            # MediaPipe's top-level import pulls in audio modules that import
            # sounddevice. On Windows this can trigger slow WMI-backed platform
            # queries that are irrelevant to pose inference.
            with _patched_platform_for_mediapipe_windows_queries():
...
class _patched_platform_for_mediapipe_windows_queries:
    def __enter__(self) -> None:
        self._uname = platform.uname
        self._win32_ver = platform.win32_ver
        ...
        platform.win32_ver = lambda *args, **kwargs: ("10", "10.0.0", "", "Multiprocessor Free")  # type: ignore[assignment]
        platform.system = lambda: "Windows"  # type: ignore[assignment]
        platform.machine = lambda: "AMD64"  # type: ignore[assignment]
        platform.architecture = lambda *args, **kwargs: ("64bit", "WindowsPE")  # type: ignore[assignment]

    def __exit__(self, exc_type, exc, tb) -> None:
        platform.uname = self._uname  # type: ignore[assignment]
        platform.win32_ver = self._win32_ver  # type: ignore[assignment]
        platform.system = self._system  # type: ignore[assignment]
        platform.machine = self._machine  # type: ignore[assignment]
        platform.architecture = self._architecture  # type: ignore[assignment]
