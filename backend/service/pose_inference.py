...
from schema.frame import ExtractedFrame, FrameChunk
...
class PoseInferenceService:
    ...
    def default_options(self) -> PoseInferenceOptions:
        return self._default_options()

    def open_session(self, options: PoseInferenceOptions | None = None) -> PoseInferenceOptions:
        resolved_options = options or self._default_options()
    ...
    def iter_infer_chunks(
        self,
        chunks: Iterable[FrameChunk],
        options: PoseInferenceOptions | None = None,
    ) -> Iterator[PoseChunk]:
        resolved_options = self.open_session(options)
        try:
            for chunk in chunks:
                yield self.infer_chunk(chunk, resolved_options)
        finally:
            self.close_session()
    ...
    def infer_chunk(
        ...
        for frame in chunk.frames:
            frame_results.append(self._infer_frame(frame, options))
            frame.image = None
        return PoseChunk(
            chunk_index=chunk.chunk_index,
            start_frame_index=chunk.start_frame_index,
            end_frame_index=chunk.end_frame_index,
            frames=frame_results,
        )
    ...
    def _infer_frame(
        ...
        mp_image_started = perf_counter()
        mp_image = self._adapter.to_mp_image(frame.image)
        mp_image_creation_ms = (perf_counter() - mp_image_started) * 1000.0
        ...
        frame_result.benchmark = PoseFrameBenchmark(
            frame_index=frame.index,
            timestamp_ms=frame.timestamp_ms,
            mp_image_creation_ms=round(mp_image_creation_ms, 3),
