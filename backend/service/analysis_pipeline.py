...
class AnalysisPipelineService:
    def analyze(
        ...
        # --- General motion experiment mode (exercise-agnostic skeleton extraction) ---
        if resolved_exercise_type == "general_motion":
            return self._analyze_general_motion(frames_raw, video_info)

        # --- Squat pipeline (preserved for future restoration) ---
    ...
    def _resolve_exercise_type(self, exercise_type: str | None) -> str:
        ...
        if normalized == "squat":
            return "squat"
        # None / empty / "general_motion" → general motion experiment mode
        return "general_motion"

    def _analyze_general_motion(self, frames_raw: list[dict], video_info: dict) -> dict:
        """Skeleton extraction only, no exercise-specific analysis."""
        clean_frames = preprocess(frames_raw)
        ...
        summary = {
            ...
            "bodyweightKg": None,
            "externalLoadKg": None,
            "barPlacementMode": None,
            "barPlacementResolved": None,
            "totalSystemMassKg": None,
        }
        return {
            "summary": summary,
            "bodyProfile": {},
